...
public class LoginViewModel extends ViewModel {
    ...
    public void login(String email, String password) {
        ...
        if (emailValue.isEmpty()) {
            authUiState.setValue(AuthUiState.error("LOGIN_EMAIL_REQUIRED"));
            return;
        }
        if (passwordValue.isEmpty()) {
            authUiState.setValue(AuthUiState.error("LOGIN_PASSWORD_REQUIRED"));
            return;
        }

        authUiState.setValue(AuthUiState.loading());
        authRepository.signInWithEmail(emailValue, passwordValue, new AuthResultCallback() {
            @Override
    ...
    public void loginWithGoogle(String idToken) {
        String tokenValue = idToken == null ? "" : idToken.trim();
        if (tokenValue.isEmpty()) {
            authUiState.setValue(AuthUiState.error("AUTH_GOOGLE_TOKEN_INVALID"));
            return;
        }

        authUiState.setValue(AuthUiState.loading());
        authRepository.signInWithGoogleIdToken(tokenValue, new AuthResultCallback() {
            @Override
            public void onSuccess() {
                authUiState.postValue(AuthUiState.success());
            }
